...
import com.vendsy.bartsy.venue.wifi.AllJoynChatInterface;
...
public class ConnectivityService extends Service implements AppObserver {
	 ...
	 * add ourselves into the list of observers of the model so we can
...
//        mWfdAutoAccept.intercept(false);
	...
	public synchronized void update(AppObservable o, Object arg) {
        ...
        if (qualifier.equals(BartsyApplication.USE_JOIN_CHANNEL_EVENT)) {
            Message message = mHandler.obtainMessage(HANDLE_USE_JOIN_CHANNEL_EVENT);
            mHandler.sendMessage(message);
        }
        
        if (qualifier.equals(BartsyApplication.USE_LEAVE_CHANNEL_EVENT)) {
            Message message = mHandler.obtainMessage(HANDLE_USE_LEAVE_CHANNEL_EVENT);
            mHandler.sendMessage(message);
        }
        
        if (qualifier.equals(BartsyApplication.HOST_INIT_CHANNEL_EVENT)) {
            Message message = mHandler.obtainMessage(HANDLE_HOST_INIT_CHANNEL_EVENT);
            mHandler.sendMessage(message);
        }
        
        if (qualifier.equals(BartsyApplication.HOST_START_CHANNEL_EVENT)) {
            Message message = mHandler.obtainMessage(HANDLE_HOST_START_CHANNEL_EVENT);
            mHandler.sendMessage(message);
        }
        
        if (qualifier.equals(BartsyApplication.HOST_STOP_CHANNEL_EVENT)) {
            Message message = mHandler.obtainMessage(HANDLE_HOST_STOP_CHANNEL_EVENT);
            mHandler.sendMessage(message);
        }
        
        if (qualifier.equals(BartsyApplication.OUTBOUND_CHANGED_EVENT)) {
    ...
    private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
	        ...
	        // We are on a host and we have just created our own channel. Connect.
    ...
    /**
     * The messages are to handle automatic connection to newly found channels 
     * - new_channel_found is for phones connecting to nearby hosts
     * - host_channel_state_change is for the tablet starting up a channel and
     * connecting to itself
     */
    private static final int HANDLE_HOST_CHANNEL_STATE_CHANGED_EVENT = 7;
    ...
    private final class BackgroundHandler extends Handler {
        ...
        public void getContacts() {
            Log.i(TAG, "mBackgroundHandler.getContacts()");
        	Message msg = mBackgroundHandler.obtainMessage(GET_CONTACTS);
        	mBackgroundHandler.sendMessage(msg);
        }
        ...
        public void hostAutoConnect() {
            ...
            case CONNECTED:
            	...
            	if (mApp.useGetChannelState() == ConnectivityService.UseChannelState.IDLE) {
                	...
                	if (lastDot < 0) {
                		Log.d(TAG, "Found channel with wrong name: " + host_name);
                		break;
                	}
...
//	        	doGetContacts();
    ...
    private static final int GET_CONTACTS = 15; // TODO
     ...
     * this object.
     */
    private BusAttachment mBus  = new BusAttachment(BartsyApplication.PACKAGE_NAME, BusAttachment.RemoteMessage.Receive);
    ...
    private class ChatBusListener extends BusListener {
		...
		public void foundAdvertisedName(String name, short transport, String namePrefix) {
			...
			BartsyApplication application = (BartsyApplication)getApplication();
    ...
    private void doBindSession() {
        ...
        Status status = mBus.bindSessionPort(contactPort, sessionOpts, new SessionPortListener() {
			...
			public void sessionJoined(short sessionPort, int id, String joiner) {
                ...
                mHostChatInterface = emitter.getInterface(AllJoynChatInterface.class);
            }             
        });
     ...
     * description of this rather non-intuitively complicated case.
     */
    AllJoynChatInterface mHostChatInterface = null;
     ...
     */
    AllJoynChatInterface mChatInterface = null;
    ...
    /**
     * TODO - Implementation of the functionality related to getting a contact list from
     * an existing remote session.
     */
    /*
    private ContactAddressBookInterface mAddressBookInterface;

    // Call AddressBookInterface.getAllContactNames and send the result to the UI handler
    private void doGetContacts() {
        Log.i(TAG, "doGetContacts()");

        if (mAddressBookInterface == null) {
        	break;
        }
        try {
            ContactNameId[] reply = mAddressBookInterface.getAllContactNames();
    ...
    */
     ...
     * AllJoyn bus object that will allow us to instantiate a signal emitter.
     */
    class ChatService implements AllJoynChatInterface, BusObject {
    	/**                                                                                                                          
