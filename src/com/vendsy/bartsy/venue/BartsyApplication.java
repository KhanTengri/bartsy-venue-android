...
public class BartsyApplication extends Application implements AppObservable {
	 ...
	 * already connected to a channel. If it's already connected it adds the 
	 * new channel to the list of channels in the main action bar UI and notifies 
	 * the user with a notification that there are other services available nearby
	 */
...
//		mHistory.add("[" + dateFormat.format(date) + "] (" + nickname + ") " + message);
	 ...
	 * the model.  Observers correspond to the view-controllers which in this
	 * case are the Android Activities (corresponding to the use tab and the
	 * Host tab) and the Android Service that does all of the AllJoyn work.
